...
def index(request: Request) -> HTMLResponse:
    return render_template(request, "index.html", {"responses": _response_files(), "outbox": str(OUTBOX_DIR)})
...
def create_prompt_pack(request: Request, task: str = Form(...), repository: str = Form(""), repo_path: str = Form(""), risk_level: str = Form("normal")) -> HTMLResponse:
    ...
    return render_template(request, "result.html", {"title": "Prompt pack creato", "content": result})
...
def ingest_response(request: Request, response: str = Form(...), response_id: str = Form("")) -> HTMLResponse:
    ...
    return render_template(
        request,
        "result.html",
        {"title": "Risposta acquisita", "content": f"Stato: {parsed.status}\nFile: {path}\nWarning: {', '.join(parsed.warnings) or 'none'}\n\n{parsed.to_json()}"},
    )
